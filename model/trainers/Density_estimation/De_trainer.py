...
def calc_z_shapes(input_size, channel_list):
    z_shapes = []

    for i in range(1,len(channel_list)):
        z_shapes.append((channel_list[i],input_size,input_size))
...
class DeTrainer(BaseTrainer):
    def __init__(self,cfg):
        self.cfg = cfg
        super(DeTrainer,self).__init__(self.cfg)
    ...
    def _train_distributed(self,loader,start_iter):
        ...
        z_sample = []
        z_shapes = calc_z_shapes(self.cfg.augmentation['kwargs']['input_size_h'], self.cfg.model['kwargs']['channel_list'])
        for z in z_shapes:
        ...
        for iter_id, (src) in enumerate(loader):
            ...
            _,log_p,output = self.model(image,z_sample)

            loss, log_p = calc_loss(log_p, self.cfg.augmentation['kwargs']['input_size_h'], self.cfg.n_bins)
            ...
            if tmp_step > 0 and tmp_step % self.cfg.save_freq == 0 and rank == 0:
                ...
                output_images = output.detach().cpu()#torch.cat((src.detach().cpu(), output.detach().cpu()), 0)
