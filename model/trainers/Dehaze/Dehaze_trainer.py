...
class DehazeTrainer(BaseTrainer):
    def __init__(self,cfg):
        self.cfg = cfg
        super(DehazeTrainer,self).__init__(self.cfg)
    ...
    def _train_distributed(self,loader,start_iter):
        ...
        tmp_step = start_iter
        for iter_id, (src, target,_,target_alt,_,_) in enumerate(loader):
            ...
            output = self.model(src,target_alt.cuda())
            ...
            if tmp_step > 0 and tmp_step % self.cfg.save_freq == 0 and rank == 0:
                ...
                self._save_checkpoint(tmp_step,self.cfg.model.arch,self.cfg.save_path+'/ckpt/'+'latest')
    ...
    def _test(self,loader):
        ...
        for iter_id, (src, target, _, _, src_path, target_path) in tqdm(enumerate(loader)):
            ...
            for idx in range(len(output)):
                ...
                #torchvision.utils.save_image(output_images, output_name, nrow=1)
                name = src_path[idx].split('/')[-1]
                folder = src_path[idx].split('/')[-4]
                pred_folder = os.path.join(self.cfg.save_path,self.cfg.pred_path,'pred',folder)
                if not os.path.exists(pred_folder):
                    os.makedirs(pred_folder)
                pred_name = os.path.join(pred_folder,name)
